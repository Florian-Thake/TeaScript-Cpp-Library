...
public:
    ...
    {
        if( mStartColumn <= 0LL || static_cast<size_t>(mStartColumn) > source.size() ) {
            throw std::out_of_range( "SourceLocation::SetSource: mStartColumn <= 0LL || mStartColumn > source.size()" );
        }
